...
public sealed record CompanionEricwToolchainStatus(
...
public static class CompanionEricwToolchainService
{
    ...
    private const string BinaryArchiveFileName =
        "ericw-tools-v0.18.1-32-g6660c5f-win64.zip";
        ...
        string managedDataRoot)
    {
                ...
                hashesPath))
        {
            throw new FileNotFoundException(
                "The Companion installation is missing its packaged ericw-tools compiler payload. " +
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidDataException(
                "The packaged ericw-tools archive failed its SHA-256 integrity check. " +
                "Reinstall or repair TrenchBroom Companion.");
        }
        ...
        try
        {
            ...
            {
                throw new InvalidDataException(
                    "The packaged ericw-tools archive does not contain qbsp.exe, vis.exe, and light.exe together.");
            }
            ...
            if (!staged.IsReady)
            {
                throw new InvalidDataException(
                    staged.Problem ??
                    "The packaged ericw-tools compiler payload is incomplete.");
            }
        ...
        if (!result.IsReady)
        {
            throw new InvalidDataException(
                result.Problem ??
                "The managed ericw-tools compiler installation is incomplete.");
        }
        ...
        string hashesPath)
    {
        ...
        throw new InvalidDataException(
            "The packaged ericw-tools SHA256SUMS.txt file does not contain a valid binary archive hash.");
    }
